...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ex287) (read-case-sensitive #t) (teachpacks ((lib "universe.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "image.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "universe.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "image.rkt" "teachpack" "2htdp")))))
...
; eval-var-lookup: BSLV AL -> Number
; Evaluate an expression e with variables assigned in association list da

(check-expect (eval-var-lookup 1 empty) 1)
(check-error (eval-var-lookup 's empty) "Undefined variable: 's")
(check-expect (eval-var-lookup (make-add 1 1) al0) 2)
(check-expect (eval-var-lookup (make-add (make-mul 2 3) 1) al0) 7)
(check-expect (eval-var-lookup (make-add 'x 1) al0) 6)
(check-expect (eval-var-lookup (make-add 'x 'y) al0) 8)
(check-error (eval-var-lookup (make-add 's 2) al0) "Undefined variable: 's")

(define (eval-var-lookup e da)
  (cond
    [(number? e) e]
    [(symbol? e) (lookup-con e da)]
